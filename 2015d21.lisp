...
(defun d21-match (&optional (shop *d21shop*))
  (let ((mincost most-positive-fixnum)
        (maxcost 0)
        minnames
        maxnames)
    (let-match (((list weps arm rings) shop))
      (let (rs
            (none (list "No Ring" 0 0 0)))
        (a:map-combinations (lambda (itm)
                              (push (cons (copy-list none) itm)
                                    rs))
                            rings
                            :length 1)
        (a:map-combinations (lambda (comb)
                            ...
                            rings
                            :length 2)
        (push (make-list 2 :initial-element (copy-list none))
              rs)
        (dolist (w weps)
          (dolist (a (cons (list "No Armor" 0 0 0) arm))
            (dolist (r rs)
              (let-match (((list w-name w-cost w-atk _) w)
                          ((list a-name a-cost _ a-arm) a)
                          ((list (list r1-name r1-cost r1-atk r1-arm)
                                 (list r2-name r2-cost r2-atk r2-arm)) r))
                (let ((c (+ w-cost a-cost r1-cost r2-cost)))
                  (if (>= (ceiling 100 (max 1 (- 8 (+ a-arm r1-arm r2-arm))))
                      ...
                      (when (> c maxcost)
                        (setf maxcost c)
                        (setf maxnames (list w-name a-name r1-name r2-name)))))))))))
    (values (list mincost minnames)
            (list maxcost maxnames))))
